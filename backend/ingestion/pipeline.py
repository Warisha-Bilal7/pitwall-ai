...
async def ingest_session_to_db(session_key: int):
    ...
    async with AsyncSessionLocal() as db:
        async with db.begin():
            try:
                print(f"\n  [Ingestion] Ingesting session {session_key} into Neon...\n")

                # 1. Fetch session metadata from OpenF1
                sessions_data = await client.get_sessions(session_key=session_key)
                if not sessions_data:
                    print(f"  [Ingestion] ERROR: No session found for key {session_key}")
                    return
                ...
                if not meeting_data:
                    print(f"  [Ingestion] ERROR: No meeting found for session {session_key}")
                    return
